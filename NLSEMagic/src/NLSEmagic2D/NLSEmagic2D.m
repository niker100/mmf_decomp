%NLSEmagic2D
%Program to integrate the two-dimensional Nonlinear Schrödinger Equation:
...
if(cuda==1)
    ...
    numcudablocksx = ceil(yres/cudablocksizex);    
    numcudablocksy = ceil(xres/cudablocksizey);
    numcudablocks  = numcudablocksx*numcudablocksy;
    if(BC==2)
        if(yres - cudablocksizex*(numcudablocksx-1) == 1)
            disp('MSD CUDA ERROR: yres is one cell greater than CUDA block,')                
            ymax = ymax-h;    
            ymin = ymin+h;
            disp(['adjusting ymax to ',num2str(ymax),' and ymin to ',num2str(ymin),' to compensate,']);                         
        end
        if(xres - cudablocksizey*(numcudablocksy-1) == 1) 
            disp('MSD CUDA ERROR: xres is one cell greater than CUDA block')
            xmax = xmax-h;   
            xmin = xmin+h; 
...
for chunk_count = 1:numframes
    if(cuda==0)
        if(precision==1)
            U = NLSE2D_TAKE_STEPS_F(U,V,s,a,h^2,BC,chunk_size,k);
        else
            U = NLSE2D_TAKE_STEPS(U,V,s,a,h^2,BC,chunk_size,k);
        end
    else
        if(precision==1)
            U = NLSE2D_TAKE_STEPS_CUDA_F(U,V,s,a,h^2,BC,chunk_size,k);
        else
            U = NLSE2D_TAKE_STEPS_CUDA(U,V,s,a,h^2,BC,chunk_size,k);
        end
    end
    n = n + chunk_size;
...
%Report wall time:
twall = toc(twall);
disp(['NLSEmagic2D Total Wall Time: ',num2str(twall),' seconds.']);
%%%%%%%%%         SIMULATION IS OVER                      %%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%